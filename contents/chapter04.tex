...
MAP-IDD provides a coded joint-detection baseline. For user a priori LLRs $L^{(t)}_{c_k,\ell,0}$, the MAP detector evaluates each candidate vector $\bm{x}^{(q)}\in\{\pm1\}^U$ using
\begin{equation}
    \Lambda_q
    =
    -\frac{\norm{\bm{y}_\ell-\bm{H}\bm{x}^{(q)}}^2}{N_0}
    +
    \frac{1}{2}\sum_{k=1}^{U}x_k^{(q)}L^{(t)}_{c_k,\ell,0}.
